...

{
...

    {
        ...
        {
            // High-Quality GL initialization for stunning visuals
            TheRender.InitHighQualityGL();
        ...
        // ——— Dark Mode Toggle (press F2 to toggle) ————————————————
        private void ApplyDarkMode(bool dark)
        {
            Globals.DarkModeEnabled = dark;
            Color bg = dark ? Color.FromArgb(30, 30, 30) : SystemColors.Control;
            Color fg = dark ? Color.White : SystemColors.ControlText;
            Color panelBg = dark ? Color.FromArgb(45, 45, 45) : SystemColors.Control;
            this.BackColor = bg;
            this.ForeColor = fg;
            ApplyDarkModeToControls(this.Controls, bg, fg, panelBg);
            glControl1.BackColor = dark ? Color.FromArgb(20, 20, 20) : Color.Black;
        }

        private void ApplyDarkModeToControls(System.Windows.Forms.Control.ControlCollection controls, Color bg, Color fg, Color panelBg)
        {
            foreach (System.Windows.Forms.Control ctrl in controls)
            {
                if (ctrl is MenuStrip || ctrl is ToolStrip || ctrl is StatusStrip)
                {
                    ...
                    if (ctrl is ToolStrip ts) { foreach (ToolStripItem item in ts.Items) { item.BackColor = bg; item.ForeColor = fg; } }
                }
                else if (ctrl is TreeView tv) { tv.BackColor = bg; tv.ForeColor = fg; }
                else if (ctrl is ListBox lb) { lb.BackColor = bg; lb.ForeColor = fg; }
                else if (ctrl is Panel || ctrl is GroupBox || ctrl is TabControl || ctrl is TabPage)
                { ctrl.BackColor = panelBg; ctrl.ForeColor = fg; }
                else if (ctrl is Button btn) { btn.BackColor = panelBg; btn.ForeColor = fg; }
                else if (ctrl is Label lbl) { lbl.ForeColor = fg; }
                else if (ctrl is TextBox tb) { tb.BackColor = bg; tb.ForeColor = fg; }
                else if (ctrl is NumericUpDown nud) { nud.BackColor = bg; nud.ForeColor = fg; }
                if (ctrl.Controls.Count > 0)
                    ApplyDarkModeToControls(ctrl.Controls, bg, fg, panelBg);
            }
        }
